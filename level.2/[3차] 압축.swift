...
// 압축 알고리즘이 영문 대문자만 처리한다고 할 때, 사전은 다음과 같이 초기화된다. 사전의 색인 번호는 정수값으로 주어지며,
// 1부터 시작한다고 하자.
...
// 2. 두 번째 글자 A는 사전에 있으나, 세 번째 글자까지인 AK는 사전에 없으므로, A의 색인 번호 1을 출력하고,
// AK를 사전에 28 번째로 등록한다.
...
func solution(_ msg:String) -> [Int] {
    ...
    while idx < msg.count {
        let char = msg[idx]
        var target = cache + char
        
        if let match = dict[target] {
            cache = target
            idx += 1
        } else {
            dict[target] = dict.count + 1
