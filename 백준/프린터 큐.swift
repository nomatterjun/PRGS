...
(1...num).forEach { _ in
  ...
  var queue = readLine()!.split(separator: " ").enumerated().map { Document(idx: $0.offset, priority: Int(String($0.element))!) }

  var result: Int = 0

  while !queue.isEmpty {
    let maxPriority = queue.map { $0.priority }.max()
    let current = queue.removeFirst()
    if current.priority == maxPriority {
      result += 1
      if current.idx == M { break }
    } else {
