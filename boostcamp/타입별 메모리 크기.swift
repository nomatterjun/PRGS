// 여러분들은 새로운 컴퓨터를 만들고 8바이트 단위로 관리하는 타입별 메모리 관리 방식을 시뮬레이션하려고 합니다.
// 지원하는 타입별 크기는 다음과 같습니다.

// BOOL 1바이트
// SHORT 2바이트
// FLOAT 4바이트
// INT 8바이트
// LONG 16바이트

// 매개변수 param0에 타입들을 입력한 순서대로 메모리를 할당한 결과를 8바이트 단위로 구분해서 return해주는 solution 함수를 작성하세요.
// 단, BOOL을 제외한 8바이트보다 작은 타입들이 연속될 경우에는 사이에 패딩(.)을 붙여야 합니다.
// SHORT는 2배수, FLOAT는 4배수가 되도록 메모리를 할당해야 합니다.

// 예를 들어 BOOL 타입 이후에 BOOL 타입은 바로 붙어서 할당할 수 있습니다.
// "BOOL", "BOOL" -> "##......"

// 그렇지만 BOOL 타입 이후에 SHORT, FLOAT를 할당하기 위해서는 각각 1개, 3개 패딩을 붙여야 합니다.
// "BOOL", "SHORT" -> "#.##...."
// "BOOL", "FLOAT" -> "#...####"
// "BOOL", "SHORT", "FLOAT" -> "#.######"

// BOOL 타입 이후에 크기가 8바이트 이상인 타입은 7개 패딩을 붙여야 합니다.
// "BOOL", "INT" -> "#.......,########"

// BOOL과 마찬가지로 8바이트보다 작은 SHORT, FLOAT도 뒤에 패딩을 붙여야 합니다.
// "SHORT", "BOOL" -> "###....."
// "FLOAT", "SHORT" -> "######.."

// - 제한 사항
// 입력 타입은 1개 이상, 100개 이하까지만 가능합니다.
// 출력하는 메모리 최대 크기는 128바이트를 기준으로 동작합니다. 따라서 128바이트보다 큰 메모리 할당은 불가능합니다.
// 만약 타입 하나라도 더 이상 할당할 수 없는 경우는 "HALT"를 리턴합니다.

func solution(_ inputs: [String]) -> String {
  // 사이즈로 매핑
  let sizes = inputs.map { DataTypes(rawValue: $0)!.size }
  // 결과로 쓰일 배열
  var results: [String] = []
  // 8 바이트 유닛
  var unit: String = ""

  sizes.forEach { size in
    // 8 바이트 유닛보다 큰 지 체크
    if unit.count + size > 8 { // 1. 크다면
      // 남은 유닛 패딩처리하고
      results.append(unit + String(repeating: ".", count: 8 - unit.count))
      // 유닛 초기화
      unit = ""
    }
    // 2. 작다면
    if size == 1 { // BOOL
      unit += "#"
    } else if size == 2 { // SHORT
      if unit.count % 2 != 0 { unit += ".##" } // 2배수가 아닐 때
      else { unit += "##" } // 2배수일 때
    } else if size == 4 { // FLOAT
      if unit.count % 4 != 0 { unit += (String(repeating: ".", count: 4 - unit.count) + "####") } // 4배수가 아닐 때
      else { unit += "####" }
    } else if size == 8 {
      results.append("########")
    } else if size == 16 {
      results.append(contentsOf: ["########", "########"])
    }
  }
  if unit.count > 0 {
    results.append(unit + String(repeating: ".", count: 8 - unit.count))
  }
  if results.count > 16 { return "HALT" }

  return results.joined(separator: ",")
}

enum DataTypes: String {
  case BOOL
  case SHORT
  case FLOAT
  case INT
  case LONG

  var size: Int {
    switch self {
    case .BOOL: return 1
    case .SHORT: return 2
    case .FLOAT: return 4
    case .INT: return 8
    case .LONG: return 16
    }
  }
}

let tests = [
  [["BOOL"], "#......."],
  [["BOOL", "BOOL"], "##......"],
  [["BOOL", "BOOL", "BOOL", "BOOL"], "####...."],
  [["BOOL", "BOOL"], "##......"],
  [["BOOL", "SHORT"], "#.##...."],
  [["BOOL", "FLOAT"], "#...####"],
  [["SHORT", "FLOAT"], "##..####"],
  [["BOOL", "SHORT", "FLOAT"], "#.######"],
  [["BOOL", "SHORT", "FLOAT", "FLOAT"], "#.######,####...."],
  [["BOOL", "INT"], "#.......,########"],
  [["SHORT", "BOOL"], "###....."],
  [["SHORT", "LONG"], "##......,########,########"],
  [["FLOAT", "SHORT"], "######.."],
  [
    ["INT", "INT", "BOOL", "SHORT", "LONG"],
    "########,########,#.##....,########,########"
  ],
  [
    ["INT", "SHORT", "FLOAT", "INT", "BOOL"],
    "########,##..####,########,#......."
  ],
  [
    ["FLOAT", "SHORT", "BOOL", "BOOL", "BOOL", "INT"],
    "########,#.......,########"
  ],
  [
    [
      "BOOL",
      "LONG",
      "SHORT",
      "LONG",
      "BOOL",
      "LONG",
      "BOOL",
      "LONG",
      "SHORT",
      "LONG",
      "LONG"
    ],
    "HALT"
  ],
  [["SHORT", "BOOL"], "###....."],
  [["INT", "SHORT", "FLOAT"], "########,##..####"],
  [
    ["INT", "INT", "BOOL", "SHORT", "LONG"],
    "########,########,#.##....,########,########"
  ],
  [
    ["INT", "SHORT", "FLOAT", "INT", "BOOL"],
    "########,##..####,########,#......."
  ],
  [
    ["FLOAT", "SHORT", "BOOL", "BOOL", "BOOL", "INT"],
    "########,#.......,########"
  ],
  [
    [
      "BOOL",
      "LONG",
      "SHORT",
      "LONG",
      "BOOL",
      "LONG",
      "BOOL",
      "LONG",
      "SHORT",
      "LONG",
      "LONG"
    ],
    "HALT"
  ]
]

tests.forEach { test in
  if solution(test[0] as! [String]) == test[1] as! String { print("Correct") }
  else { print("Wrong") }
}