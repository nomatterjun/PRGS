...
func solution(_ new_id:String) -> String {
    ...
    // step 3
    id = id.removeRepeatingDots()
    
    // step 4
    id = id.removeEdgeDots()
    
    // step 5
    if id.isEmpty { id = "a" }
    
    // step 6
    if id.count >= 16 { id = String(id.prefix(15)) }
    id = id.removeEdgeDots()
    
    // step 7
    while id.count <= 2 {
        id += String(id.suffix(1))
    }
...
extension String {
    func removeSymbols() -> String {
        return self.reduce("") { result, char in
            if !char.isLetter && !char.isNumber { // 특수문자
                if char != "-" && char != "_" && char != "." {
                    return result
                }
            }
    ...
    func removeRepeatingDots() -> String {
        return self.reduce("") { result, char in
            if result.suffix(1) == "." {
                if char == "." {
                    return result
