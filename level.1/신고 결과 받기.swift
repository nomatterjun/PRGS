...
func solution(_ id_list:[String], _ report:[String], _ k:Int) -> [Int] {
    var reportedIDs: Dictionary<String, Set<String>> = [:]
    
    for rep in report {
        let split = rep.split(separator: " ").map { String($0) }
        let (reporter, reported) = (split[0], split[1])
        reportedIDs[reported, default: Set<String>()].insert(reporter)
    }
    
    let bannedIDs = reportedIDs.filter { $0.value.count >= k }
    
    var result: [String: Int] = [:]
    bannedIDs.forEach { id in
        id.value.forEach { v in
            result[v, default: 0] += 1
        }
    }
