...
func solution(_ dartResult:String) -> Int {
    ...
    dartResult.forEach {
        if $0.isNumber {
            ...
            else { cache = Int(String($0))! }
        } else if $0.isLetter {
            scores.append(
                Int(pow(Double(cache), Bonus(rawValue: String($0))!.mult))
            )
            cache = 0
        } else if $0 == "*" {
            for index in scores.endIndex - 2...scores.endIndex - 1 where index >= 0 {
                scores[index] *= 2
            }
        } else if $0 == "#" {
