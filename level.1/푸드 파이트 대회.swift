...
func solution(_ food:[Int]) -> String {
    let foodWithoutWater = Array(food.dropFirst())
    let arr: [String] = foodWithoutWater.enumerated().map { idx, f in
        return String(repeating: "\(idx + 1)", count: f / 2)
    }
    let result = String(arr.joined())
